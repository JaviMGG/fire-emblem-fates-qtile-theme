...
if [ ! -f "$HOME/.config/qtile/fates_wallpaper.jpg" ]; then
    log_message "Error: Wallpaper file fates_wallpaper.jpg not found"
    log_message "Creating a backup file..."
    touch "$HOME/.config/qtile/fates_wallpaper.jpg"
fi

# Set wallpaper
if command -v feh &> /dev/null; then
    log_message "Setting wallpaper with feh"
    feh --bg-fill "$HOME/.config/qtile/fates_wallpaper.jpg" || log_message "Error setting wallpaper"
else
